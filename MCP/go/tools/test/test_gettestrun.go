...
func Test_gettestrunHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
...
func CreateTest_gettestrunTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("get_v0_1_apps_owner_name_app_name_test_runs_test_run_id",
		mcp.WithDescription("Returns a single test runs"),
	...
	return models.Tool{
		Definition: tool,
		Handler:    Test_gettestrunHandler(cfg),
	}
}
