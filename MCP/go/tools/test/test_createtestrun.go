...
func Test_createtestrunHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	return func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		url := fmt.Sprintf("%s/v0.1/apps/%s/%s/test_runs", cfg.BaseURL, owner_name, app_name)
...
func CreateTest_createtestrunTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("post_v0_1_apps_owner_name_app_name_test_runs",
		mcp.WithDescription("Creates a new test run"),
	...
	return models.Tool{
		Definition: tool,
		Handler:    Test_createtestrunHandler(cfg),
	}
}
