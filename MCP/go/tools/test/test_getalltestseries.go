...
func Test_getalltestseriesHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	return func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		url := fmt.Sprintf("%s/v0.1/apps/%s/%s/test_series%s", cfg.BaseURL, owner_name, app_name, queryString)
...
func CreateTest_getalltestseriesTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("get_v0_1_apps_owner_name_app_name_test_series",
		mcp.WithDescription("Returns list of all test series for an application"),
		mcp.WithString("query", mcp.Description("A query string to filter test series")),
	...
	return models.Tool{
		Definition: tool,
		Handler:    Test_getalltestseriesHandler(cfg),
	}
}
