...
func Test_deletedevicesetofownerHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
...
func CreateTest_deletedevicesetofownerTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("delete_v0_1_apps_owner_name_app_name_owner_device_sets_id",
		mcp.WithDescription("Deletes a device set belonging to the owner"),
	...
	return models.Tool{
		Definition: tool,
		Handler:    Test_deletedevicesetofownerHandler(cfg),
	}
}
