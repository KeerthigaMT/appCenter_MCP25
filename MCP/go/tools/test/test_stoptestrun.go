...
func Test_stoptestrunHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	return func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		url := fmt.Sprintf("%s/v0.1/apps/%s/%s/test_runs/%s/stop", cfg.BaseURL, test_run_id, owner_name, app_name)
...
func CreateTest_stoptestrunTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("put_v0_1_apps_owner_name_app_name_test_runs_test_run_id_stop",
		mcp.WithDescription("Stop a test run execution"),
		mcp.WithString("test_run_id", mcp.Required(), mcp.Description("The ID of the test run to be stopped")),
	...
	return models.Tool{
		Definition: tool,
		Handler:    Test_stoptestrunHandler(cfg),
	}
}
