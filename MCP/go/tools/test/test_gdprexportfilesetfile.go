...
func Test_gdprexportfilesetfileHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	return func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		url := fmt.Sprintf("%s/v0.1/apps/%s/%s/test/export/fileSetFiles", cfg.BaseURL, owner_name, app_name)
...
func CreateTest_gdprexportfilesetfileTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("get_v0_1_apps_owner_name_app_name_test_export_fileSetFiles",
		mcp.WithDescription("Lists file set file data"),
	...
	return models.Tool{
		Definition: tool,
		Handler:    Test_gdprexportfilesetfileHandler(cfg),
	}
}
