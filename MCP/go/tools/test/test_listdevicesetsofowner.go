...
func Test_listdevicesetsofownerHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	return func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		url := fmt.Sprintf("%s/v0.1/apps/%s/%s/owner/device_sets", cfg.BaseURL, owner_name, app_name)
...
func CreateTest_listdevicesetsofownerTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("get_v0_1_apps_owner_name_app_name_owner_device_sets",
		mcp.WithDescription("Lists device sets belonging to the owner"),
	...
	return models.Tool{
		Definition: tool,
		Handler:    Test_listdevicesetsofownerHandler(cfg),
	}
}
