...
func Test_gdprexporttestrunHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	return func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		url := fmt.Sprintf("%s/v0.1/apps/%s/%s/test/export/testRuns", cfg.BaseURL, owner_name, app_name)
...
func CreateTest_gdprexporttestrunTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("get_v0_1_apps_owner_name_app_name_test_export_testRuns",
		mcp.WithDescription("Lists test run data"),
	...
	return models.Tool{
		Definition: tool,
		Handler:    Test_gdprexporttestrunHandler(cfg),
	}
}
