...
func Test_createdeviceselectionHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	return func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		url := fmt.Sprintf("%s/v0.1/apps/%s/%s/device_selection", cfg.BaseURL, owner_name, app_name)
...
func CreateTest_createdeviceselectionTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("post_v0_1_apps_owner_name_app_name_device_selection",
		mcp.WithDescription("Creates a short ID for a list of devices"),
		...
		mcp.WithArray("devices", mcp.Required(), mcp.Description("")),
	)

	return models.Tool{
		Definition: tool,
		Handler:    Test_createdeviceselectionHandler(cfg),
	}
}
