...
func Test_gettestreportHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	return func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		url := fmt.Sprintf("%s/v0.1/apps/%s/%s/test_runs/%s/report", cfg.BaseURL, test_run_id, owner_name, app_name)
...
func CreateTest_gettestreportTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("get_v0_1_apps_owner_name_app_name_test_runs_test_run_id_report",
		mcp.WithDescription("Returns a single test report"),
	...
	return models.Tool{
		Definition: tool,
		Handler:    Test_gettestreportHandler(cfg),
	}
}
