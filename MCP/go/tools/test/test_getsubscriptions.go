...
func Test_getsubscriptionsHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	return func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		url := fmt.Sprintf("%s/v0.1/apps/%s/%s/subscriptions", cfg.BaseURL, owner_name, app_name)
...
func CreateTest_getsubscriptionsTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("get_v0_1_apps_owner_name_app_name_subscriptions",
		mcp.WithDescription("Get information about the currently active subscriptions, if any"),
	...
	return models.Tool{
		Definition: tool,
		Handler:    Test_getsubscriptionsHandler(cfg),
	}
}
