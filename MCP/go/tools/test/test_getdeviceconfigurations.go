...
func Test_getdeviceconfigurationsHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	return func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		if val, ok := args["app_upload_id"]; ok {
			queryParams = append(queryParams, fmt.Sprintf("app_upload_id=%v", val))
		}
		queryString := ""
		if len(queryParams) > 0 {
			queryString = "?" + strings.Join(queryParams, "&")
		}
		url := fmt.Sprintf("%s/v0.1/apps/%s/%s/device_configurations%s", cfg.BaseURL, owner_name, app_name, queryString)
...
func CreateTest_getdeviceconfigurationsTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("get_v0_1_apps_owner_name_app_name_device_configurations",
		mcp.WithDescription("Returns a list of available devices"),
		mcp.WithString("app_upload_id", mcp.Description("The ID of the test run")),
	...
	return models.Tool{
		Definition: tool,
		Handler:    Test_getdeviceconfigurationsHandler(cfg),
	}
}
