...
func Test_archivetestrunHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	return func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		url := fmt.Sprintf("%s/v0.1/apps/%s/%s/test_runs/%s", cfg.BaseURL, test_run_id, owner_name, app_name)
...
func CreateTest_archivetestrunTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("delete_v0_1_apps_owner_name_app_name_test_runs_test_run_id",
		mcp.WithDescription("Logically deletes a test run"),
	...
	return models.Tool{
		Definition: tool,
		Handler:    Test_archivetestrunHandler(cfg),
	}
}
