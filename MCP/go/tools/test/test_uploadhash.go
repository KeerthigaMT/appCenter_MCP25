...
func Test_uploadhashHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	return func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		url := fmt.Sprintf("%s/v0.1/apps/%s/%s/test_runs/%s/hashes", cfg.BaseURL, test_run_id, owner_name, app_name)
...
func CreateTest_uploadhashTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("post_v0_1_apps_owner_name_app_name_test_runs_test_run_id_hashes",
		mcp.WithDescription("Adds file with the given hash to a test run"),
		...
		mcp.WithString("byte_range", mcp.Description("Input parameter: Range of bytes required to verify ownership of the file")),
		mcp.WithString("checksum", mcp.Required(), mcp.Description("Input parameter: SHA256 hash of the file")),
		mcp.WithString("file_type", mcp.Required(), mcp.Description("Input parameter: Type of the file")),
		mcp.WithString("relative_path", mcp.Required(), mcp.Description("Input parameter: Relative path of the file")),
	)

	return models.Tool{
		Definition: tool,
		Handler:    Test_uploadhashHandler(cfg),
	}
}
