...
func Test_createdevicesetofuserHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	return func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		url := fmt.Sprintf("%s/v0.1/apps/%s/%s/user/device_sets", cfg.BaseURL, owner_name, app_name)
...
func CreateTest_createdevicesetofuserTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("post_v0_1_apps_owner_name_app_name_user_device_sets",
		mcp.WithDescription("Creates a device set belonging to the user"),
	...
	return models.Tool{
		Definition: tool,
		Handler:    Test_createdevicesetofuserHandler(cfg),
	}
}
