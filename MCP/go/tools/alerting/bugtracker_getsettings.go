...
func Bugtracker_getsettingsHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	return func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		url := fmt.Sprintf("%s/v0.1/apps/%s/%s/bugtracker", cfg.BaseURL, owner_name, app_name)
...
func CreateBugtracker_getsettingsTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("get_v0_1_apps_owner_name_app_name_bugtracker",
		mcp.WithDescription("Get bug tracker settings for a particular app"),
	...
	return models.Tool{
		Definition: tool,
		Handler:    Bugtracker_getsettingsHandler(cfg),
	}
}
