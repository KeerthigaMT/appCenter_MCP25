...
func Notifications_getappemailsettingsHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	return func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		url := fmt.Sprintf("%s/v0.1/apps/%s/%s/notifications/emailSettings", cfg.BaseURL, owner_name, app_name)
...
func CreateNotifications_getappemailsettingsTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("get_v0_1_apps_owner_name_app_name_notifications_emailSettings",
		mcp.WithDescription("Get Email notification settings of user for a particular app"),
	...
	return models.Tool{
		Definition: tool,
		Handler:    Notifications_getappemailsettingsHandler(cfg),
	}
}
