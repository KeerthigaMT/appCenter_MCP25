...
func Webhooks_listHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	return func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		url := fmt.Sprintf("%s/v0.1/apps/%s/%s/webhooks", cfg.BaseURL, owner_name, app_name)
...
func CreateWebhooks_listTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("get_v0_1_apps_owner_name_app_name_webhooks",
		mcp.WithDescription("Get web hooks configured for a particular app"),
	...
	return models.Tool{
		Definition: tool,
		Handler:    Webhooks_listHandler(cfg),
	}
}
