...
func Bugtracker_getrepoissuefromcrashHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	return func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		url := fmt.Sprintf("%s/v0.1/apps/%s/%s/bugtracker/crashGroup/%s", cfg.BaseURL, crash_group_id, owner_name, app_name)
...
func CreateBugtracker_getrepoissuefromcrashTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("get_v0_1_apps_owner_name_app_name_bugtracker_crashGroup_crash_group_id",
		mcp.WithDescription("Get project issue related to a crash group"),
		mcp.WithString("crash_group_id", mcp.Required(), mcp.Description("CrashGroup Id")),
	...
	return models.Tool{
		Definition: tool,
		Handler:    Bugtracker_getrepoissuefromcrashHandler(cfg),
	}
}
