...
func Builds_getlogHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	return func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		url := fmt.Sprintf("%s/v0.1/apps/%s/%s/builds/%s/logs", cfg.BaseURL, build_id, owner_name, app_name)
...
func CreateBuilds_getlogTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("get_v0_1_apps_owner_name_app_name_builds_build_id_logs",
		mcp.WithDescription("Get the build log"),
	...
	return models.Tool{
		Definition: tool,
		Handler:    Builds_getlogHandler(cfg),
	}
}
