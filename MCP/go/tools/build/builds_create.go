...
func Builds_createHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	return func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		url := fmt.Sprintf("%s/v0.1/apps/%s/%s/branches/%s/builds", cfg.BaseURL, branch, owner_name, app_name)
...
func CreateBuilds_createTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("post_v0_1_apps_owner_name_app_name_branches_branch_builds",
		mcp.WithDescription("Create a build"),
		...
		mcp.WithBoolean("debug", mcp.Description("Input parameter: Run build in debug mode")),
		mcp.WithString("sourceVersion", mcp.Description("Input parameter: Version to build which represents the full Git commit reference")),
	)

	return models.Tool{
		Definition: tool,
		Handler:    Builds_createHandler(cfg),
	}
}
