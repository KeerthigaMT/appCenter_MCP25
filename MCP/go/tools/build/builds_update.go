...
func Builds_updateHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
...
func CreateBuilds_updateTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("patch_v0_1_apps_owner_name_app_name_builds_build_id",
		mcp.WithDescription("Cancels a build"),
		...
		mcp.WithString("status", mcp.Description("Input parameter: The build status; used to cancel builds")),
	)

	return models.Tool{
		Definition: tool,
		Handler:    Builds_updateHandler(cfg),
	}
}
