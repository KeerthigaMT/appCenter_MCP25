...
func Repositoryconfigurations_listHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	return func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		url := fmt.Sprintf("%s/v0.1/apps/%s/%s/repo_config%s", cfg.BaseURL, owner_name, app_name, queryString)
...
func CreateRepositoryconfigurations_listTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("get_v0_1_apps_owner_name_app_name_repo_config",
		mcp.WithDescription("Returns the repository build configuration status of the app"),
		mcp.WithBoolean("includeInactive", mcp.Description("Include inactive configurations if none are active")),
	...
	return models.Tool{
		Definition: tool,
		Handler:    Repositoryconfigurations_listHandler(cfg),
	}
}
