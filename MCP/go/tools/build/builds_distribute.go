...
func Builds_distributeHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	return func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		url := fmt.Sprintf("%s/v0.1/apps/%s/%s/builds/%s/distribute", cfg.BaseURL, build_id, owner_name, app_name)
...
func CreateBuilds_distributeTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("post_v0_1_apps_owner_name_app_name_builds_build_id_distribute",
		mcp.WithDescription("Distribute a build"),
		...
		mcp.WithString("releaseNotes", mcp.Description("Input parameter: The release notes")),
		mcp.WithArray("destinations", mcp.Description("Input parameter: Array of objects {id:string, type:string} with \"id\" being the distribution group ID, store ID, or tester email, and \"type\" being \"group\", \"store\", or \"tester\"")),
		mcp.WithBoolean("mandatoryUpdate", mcp.Description("")),
		mcp.WithBoolean("notifyTesters", mcp.Description("")),
	)

	return models.Tool{
		Definition: tool,
		Handler:    Builds_distributeHandler(cfg),
	}
}
