...
func Builds_listxcodeversionsHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	return func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		url := fmt.Sprintf("%s/v0.1/apps/%s/%s/xcode_versions", cfg.BaseURL, owner_name, app_name)
...
func CreateBuilds_listxcodeversionsTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("get_v0_1_apps_owner_name_app_name_xcode_versions",
		mcp.WithDescription("Gets the Xcode versions available to this app"),
	...
	return models.Tool{
		Definition: tool,
		Handler:    Builds_listxcodeversionsHandler(cfg),
	}
}
