...
func Builds_listtoolsetsHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	return func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		if val, ok := args["tools"]; ok {
			queryParams = append(queryParams, fmt.Sprintf("tools=%v", val))
		}
		queryString := ""
		if len(queryParams) > 0 {
			queryString = "?" + strings.Join(queryParams, "&")
		}
		url := fmt.Sprintf("%s/v0.1/apps/%s/%s/toolsets%s", cfg.BaseURL, owner_name, app_name, queryString)
...
func CreateBuilds_listtoolsetsTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("get_v0_1_apps_owner_name_app_name_toolsets",
		mcp.WithDescription("Returns available toolsets for application"),
		mcp.WithString("tools", mcp.Description("Toolset name")),
	...
	return models.Tool{
		Definition: tool,
		Handler:    Builds_listtoolsetsHandler(cfg),
	}
}
