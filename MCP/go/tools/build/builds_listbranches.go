...
func Builds_listbranchesHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	return func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		url := fmt.Sprintf("%s/v0.1/apps/%s/%s/branches", cfg.BaseURL, owner_name, app_name)
...
func CreateBuilds_listbranchesTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("get_v0_1_apps_owner_name_app_name_branches",
		mcp.WithDescription("Returns the list of Git branches for this application"),
	...
	return models.Tool{
		Definition: tool,
		Handler:    Builds_listbranchesHandler(cfg),
	}
}
