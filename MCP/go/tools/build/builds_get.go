...
func Builds_getHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	return func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		url := fmt.Sprintf("%s/v0.1/apps/%s/%s/builds/%s", cfg.BaseURL, build_id, owner_name, app_name)
...
func CreateBuilds_getTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("get_v0_1_apps_owner_name_app_name_builds_build_id",
		mcp.WithDescription("Returns the build detail for the given build ID"),
