...
func Builds_getstatusbyappidHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	return func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		url := fmt.Sprintf("%s/v0.1/apps/%s/%s/build_service_status", cfg.BaseURL, owner_name, app_name)
...
func CreateBuilds_getstatusbyappidTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("get_v0_1_apps_owner_name_app_name_build_service_status",
		mcp.WithDescription("Application specific build service status"),
	...
	return models.Tool{
		Definition: tool,
		Handler:    Builds_getstatusbyappidHandler(cfg),
	}
}
