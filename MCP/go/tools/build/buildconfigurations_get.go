...
func Buildconfigurations_getHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	return func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		url := fmt.Sprintf("%s/v0.1/apps/%s/%s/branches/%s/export_config%s", cfg.BaseURL, branch, owner_name, app_name, queryString)
...
func CreateBuildconfigurations_getTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("get_v0_1_apps_owner_name_app_name_branches_branch_export_config",
		mcp.WithDescription("Gets the build configuration in Azure pipeline YAML format"),
		mcp.WithString("branch", mcp.Required(), mcp.Description("The branch name")),
		mcp.WithString("format", mcp.Description("Configuration format")),
	...
	return models.Tool{
		Definition: tool,
		Handler:    Buildconfigurations_getHandler(cfg),
	}
}
