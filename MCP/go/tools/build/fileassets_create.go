...
func Fileassets_createHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	return func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		url := fmt.Sprintf("%s/v0.1/apps/%s/%s/file_asset", cfg.BaseURL, owner_name, app_name)
...
func CreateFileassets_createTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("post_v0_1_apps_owner_name_app_name_file_asset",
		mcp.WithDescription("Create a new asset to upload a file"),
	...
	return models.Tool{
		Definition: tool,
		Handler:    Fileassets_createHandler(cfg),
	}
}
