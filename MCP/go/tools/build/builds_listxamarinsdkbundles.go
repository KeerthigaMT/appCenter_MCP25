...
func Builds_listxamarinsdkbundlesHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	return func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		url := fmt.Sprintf("%s/v0.1/apps/%s/%s/xamarin_sdk_bundles", cfg.BaseURL, owner_name, app_name)
...
func CreateBuilds_listxamarinsdkbundlesTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("get_v0_1_apps_owner_name_app_name_xamarin_sdk_bundles",
		mcp.WithDescription("Gets the Xamarin SDK bundles available to this app"),
	...
	return models.Tool{
		Definition: tool,
		Handler:    Builds_listxamarinsdkbundlesHandler(cfg),
	}
}
