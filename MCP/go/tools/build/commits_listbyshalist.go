...
func Commits_listbyshalistHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	return func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		if val, ok := args["hashes"]; ok {
			queryParams = append(queryParams, fmt.Sprintf("hashes=%v", val))
		}
		queryString := ""
		if len(queryParams) > 0 {
			queryString = "?" + strings.Join(queryParams, "&")
		}
		url := fmt.Sprintf("%s/v0.1/apps/%s/%s/commits/batch%s", cfg.BaseURL, owner_name, app_name, queryString)
...
func CreateCommits_listbyshalistTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("get_v0_1_apps_owner_name_app_name_commits_batch",
		mcp.WithDescription("Returns commit information for a batch of shas"),
		mcp.WithArray("hashes", mcp.Required(), mcp.Description("A collection of commit SHAs comma-delimited")),
	...
	return models.Tool{
		Definition: tool,
		Handler:    Commits_listbyshalistHandler(cfg),
	}
}
