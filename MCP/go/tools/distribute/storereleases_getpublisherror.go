...
func Storereleases_getpublisherrorHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	return func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		url := fmt.Sprintf("%s/v0.1/apps/%s/%s/distribution_stores/%s/releases/%s/publish_error_details", cfg.BaseURL, store_name, release_id, owner_name, app_name)
...
func CreateStorereleases_getpublisherrorTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("get_v0_1_apps_owner_name_app_name_distribution_stores_store_name_releases_release_id_publish_error_details",
		mcp.WithDescription("Return the Error Details of release which failed in publishing."),
	...
	return models.Tool{
		Definition: tool,
		Handler:    Storereleases_getpublisherrorHandler(cfg),
	}
}
