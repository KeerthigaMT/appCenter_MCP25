...
func Applemapping_getHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
...
func CreateApplemapping_getTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("get_v0_1_apps_owner_name_app_name_apple_mapping",
		mcp.WithDescription("Get mapping of apple app to an existing app in apple store."),
	...
	return models.Tool{
		Definition: tool,
		Handler:    Applemapping_getHandler(cfg),
	}
}
