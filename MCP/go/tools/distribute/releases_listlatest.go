...
func Releases_listlatestHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	return func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		url := fmt.Sprintf("%s/v0.1/apps/%s/%s/recent_releases", cfg.BaseURL, owner_name, app_name)
...
func CreateReleases_listlatestTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("get_v0_1_apps_owner_name_app_name_recent_releases",
		mcp.WithDescription("Get the latest release from every distribution group associated with an application."),
	...
	return models.Tool{
		Definition: tool,
		Handler:    Releases_listlatestHandler(cfg),
	}
}
