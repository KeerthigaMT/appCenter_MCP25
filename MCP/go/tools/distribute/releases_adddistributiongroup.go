...
func Releases_adddistributiongroupHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	return func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		url := fmt.Sprintf("%s/v0.1/apps/%s/%s/releases/%s/groups", cfg.BaseURL, release_id, owner_name, app_name)
...
func CreateReleases_adddistributiongroupTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("post_v0_1_apps_owner_name_app_name_releases_release_id_groups",
		mcp.WithDescription("Distributes a release to a group"),
		...
		mcp.WithString("id", mcp.Description("Input parameter: Unique id of the release destination")),
		mcp.WithBoolean("mandatory_update", mcp.Description("Input parameter: Flag to mark the release for the provided destinations as mandatory")),
		mcp.WithBoolean("notify_testers", mcp.Description("Input parameter: Flag to enable or disable notifications to testers")),
	)

	return models.Tool{
		Definition: tool,
		Handler:    Releases_adddistributiongroupHandler(cfg),
	}
}
