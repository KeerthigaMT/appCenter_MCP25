...
func Storenotifications_getnotificationbyappidHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	return func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		url := fmt.Sprintf("%s/v0.1/apps/%s/%s/store_service_status", cfg.BaseURL, owner_name, app_name)
...
func CreateStorenotifications_getnotificationbyappidTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("get_v0_1_apps_owner_name_app_name_store_service_status",
		mcp.WithDescription("Application specific store service status"),
	...
	return models.Tool{
		Definition: tool,
		Handler:    Storenotifications_getnotificationbyappidHandler(cfg),
	}
}
