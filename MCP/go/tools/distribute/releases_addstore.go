...
func Releases_addstoreHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	return func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		url := fmt.Sprintf("%s/v0.1/apps/%s/%s/releases/%s/stores", cfg.BaseURL, release_id, owner_name, app_name)
...
func CreateReleases_addstoreTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("post_v0_1_apps_owner_name_app_name_releases_release_id_stores",
		mcp.WithDescription("Distributes a release to a store"),
		...
		mcp.WithString("id", mcp.Required(), mcp.Description("Input parameter: Unique id of the release destination")),
	)

	return models.Tool{
		Definition: tool,
		Handler:    Releases_addstoreHandler(cfg),
	}
}
