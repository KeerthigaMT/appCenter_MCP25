...
func Storereleases_listHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	return func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		url := fmt.Sprintf("%s/v0.1/apps/%s/%s/distribution_stores/%s/releases", cfg.BaseURL, store_name, owner_name, app_name)
...
func CreateStorereleases_listTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("get_v0_1_apps_owner_name_app_name_distribution_stores_store_name_releases",
		mcp.WithDescription("Return all releases published  in a store"),
	...
	return models.Tool{
		Definition: tool,
		Handler:    Storereleases_listHandler(cfg),
	}
}
