...
func Stores_patchHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	return func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		url := fmt.Sprintf("%s/v0.1/apps/%s/%s/distribution_stores/%s", cfg.BaseURL, store_name, owner_name, app_name)
...
func CreateStores_patchTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("patch_v0_1_apps_owner_name_app_name_distribution_stores_store_name",
		mcp.WithDescription("Update the store."),
		...
		mcp.WithString("service_connection_id", mcp.Required(), mcp.Description("Input parameter: Service connection id to updated.")),
	)

	return models.Tool{
		Definition: tool,
		Handler:    Stores_patchHandler(cfg),
	}
}
