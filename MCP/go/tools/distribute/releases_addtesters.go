...
func Releases_addtestersHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	return func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		url := fmt.Sprintf("%s/v0.1/apps/%s/%s/releases/%s/testers", cfg.BaseURL, release_id, owner_name, app_name)
...
func CreateReleases_addtestersTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("post_v0_1_apps_owner_name_app_name_releases_release_id_testers",
		mcp.WithDescription("Distributes a release to a user"),
		...
		mcp.WithString("email", mcp.Required(), mcp.Description("Input parameter: Tester's email address")),
	...
	return models.Tool{
		Definition: tool,
		Handler:    Releases_addtestersHandler(cfg),
	}
}
