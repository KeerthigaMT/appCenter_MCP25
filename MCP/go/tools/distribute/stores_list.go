...
func Stores_listHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
...
func CreateStores_listTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("get_v0_1_apps_owner_name_app_name_distribution_stores",
		mcp.WithDescription("Get all the store details from Storage store table for a particular application."),
	...
	return models.Tool{
		Definition: tool,
		Handler:    Stores_listHandler(cfg),
	}
}
