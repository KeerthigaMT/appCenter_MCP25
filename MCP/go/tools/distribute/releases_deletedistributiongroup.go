...
func Releases_deletedistributiongroupHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
...
func CreateReleases_deletedistributiongroupTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("delete_v0_1_apps_owner_name_app_name_releases_release_id_groups_group_id",
		mcp.WithDescription("Delete the given distribution group from the release"),
		mcp.WithNumber("release_id", mcp.Required(), mcp.Description("The ID of the release")),
		mcp.WithString("group_id", mcp.Required(), mcp.Description("The id of the distribution group")),
	...
	return models.Tool{
		Definition: tool,
		Handler:    Releases_deletedistributiongroupHandler(cfg),
	}
}
