...
func Devices_listHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	return func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		if val, ok := args["release_id"]; ok {
			queryParams = append(queryParams, fmt.Sprintf("release_id=%v", val))
		}
		queryString := ""
		if len(queryParams) > 0 {
			queryString = "?" + strings.Join(queryParams, "&")
		}
		url := fmt.Sprintf("%s/v0.1/apps/%s/%s/distribution_groups/%s/devices%s", cfg.BaseURL, distribution_group_name, owner_name, app_name, queryString)
...
func CreateDevices_listTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("get_v0_1_apps_owner_name_app_name_distribution_groups_distribution_group_name_devices",
		mcp.WithDescription("Returns all devices associated with the given distribution group"),
		mcp.WithString("distribution_group_name", mcp.Required(), mcp.Description("The name of the distribution group.")),
		mcp.WithString("release_id", mcp.Description("when provided, gets the provisioning state of the devices owned by users of this distribution group when compared to the provided release.")),
	...
	return models.Tool{
		Definition: tool,
		Handler:    Devices_listHandler(cfg),
	}
}
