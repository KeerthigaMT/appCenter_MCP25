...
func Applemapping_testflightgroupsHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	return func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		url := fmt.Sprintf("%s/v0.1/apps/%s/%s/apple_test_flight_groups", cfg.BaseURL, owner_name, app_name)
...
func CreateApplemapping_testflightgroupsTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("get_v0_1_apps_owner_name_app_name_apple_test_flight_groups",
		mcp.WithDescription("Fetch all apple test flight groups"),
	...
	return models.Tool{
		Definition: tool,
		Handler:    Applemapping_testflightgroupsHandler(cfg),
	}
}
