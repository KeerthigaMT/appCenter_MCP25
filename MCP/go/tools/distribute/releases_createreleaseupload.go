...
func Releases_createreleaseuploadHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	return func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		url := fmt.Sprintf("%s/v0.1/apps/%s/%s/uploads/releases", cfg.BaseURL, owner_name, app_name)
...
func CreateReleases_createreleaseuploadTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("post_v0_1_apps_owner_name_app_name_uploads_releases",
		mcp.WithDescription("Initiate a new release upload. This API is part of multi-step upload process."),
		...
		mcp.WithString("build_number", mcp.Description("Input parameter: User defined build number")),
		mcp.WithString("build_version", mcp.Description("Input parameter: User defined build version")),
	)

	return models.Tool{
		Definition: tool,
		Handler:    Releases_createreleaseuploadHandler(cfg),
	}
}
