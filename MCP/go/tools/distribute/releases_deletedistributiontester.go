...
func Releases_deletedistributiontesterHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
...
func CreateReleases_deletedistributiontesterTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("delete_v0_1_apps_owner_name_app_name_releases_release_id_testers_tester_id",
		mcp.WithDescription("Delete the given tester from the release"),
	...
	return models.Tool{
		Definition: tool,
		Handler:    Releases_deletedistributiontesterHandler(cfg),
	}
}
