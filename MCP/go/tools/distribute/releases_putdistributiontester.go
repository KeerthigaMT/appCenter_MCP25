...
func Releases_putdistributiontesterHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	return func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		tester_idVal, ok := args["tester_id"]
		if !ok {
			return mcp.NewToolResultError("Missing required path parameter: tester_id"), nil
		}
		tester_id, ok := tester_idVal.(string)
		if !ok {
			return mcp.NewToolResultError("Invalid path parameter: tester_id"), nil
		}
		...
		url := fmt.Sprintf("%s/v0.1/apps/%s/%s/releases/%s/testers/%s", cfg.BaseURL, release_id, tester_id, owner_name, app_name)
...
func CreateReleases_putdistributiontesterTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("put_v0_1_apps_owner_name_app_name_releases_release_id_testers_tester_id",
		mcp.WithDescription("Update details about the specified tester associated with the release"),
		mcp.WithNumber("release_id", mcp.Required(), mcp.Description("The ID of the release")),
		mcp.WithString("tester_id", mcp.Required(), mcp.Description("The id of the tester")),
	...
	return models.Tool{
		Definition: tool,
		Handler:    Releases_putdistributiontesterHandler(cfg),
	}
}
