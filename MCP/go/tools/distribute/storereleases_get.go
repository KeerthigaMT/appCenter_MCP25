...
func Storereleases_getHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	return func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		url := fmt.Sprintf("%s/v0.1/apps/%s/%s/distribution_stores/%s/releases/%s", cfg.BaseURL, store_name, release_id, owner_name, app_name)
...
func CreateStorereleases_getTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("get_v0_1_apps_owner_name_app_name_distribution_stores_store_name_releases_release_id",
		mcp.WithDescription("Return releases published in a store for releaseId and storeId"),
		mcp.WithString("store_name", mcp.Required(), mcp.Description("The name of the store")),
		mcp.WithString("release_id", mcp.Required(), mcp.Description("The name of the store")),
	...
	return models.Tool{
		Definition: tool,
		Handler:    Storereleases_getHandler(cfg),
	}
}
