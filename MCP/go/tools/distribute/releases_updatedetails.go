...
func Releases_updatedetailsHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	return func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		url := fmt.Sprintf("%s/v0.1/apps/%s/%s/releases/%s", cfg.BaseURL, release_id, owner_name, app_name)
...
func CreateReleases_updatedetailsTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("put_v0_1_apps_owner_name_app_name_releases_release_id",
		mcp.WithDescription("Update details of a release."),
		...
		mcp.WithBoolean("enabled", mcp.Description("Input parameter: Toggle this release to be enable distribute/download or not.")),
		mcp.WithString("release_notes", mcp.Description("Input parameter: Release notes for this release.")),
		mcp.WithObject("build", mcp.Description("Input parameter: Contains metadata about the build that produced the release being uploaded")),
	)

	return models.Tool{
		Definition: tool,
		Handler:    Releases_updatedetailsHandler(cfg),
	}
}
