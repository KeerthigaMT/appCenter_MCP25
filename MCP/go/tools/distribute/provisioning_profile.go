...
func Provisioning_profileHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	return func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		url := fmt.Sprintf("%s/v0.1/apps/%s/%s/releases/%s/provisioning_profile", cfg.BaseURL, release_id, owner_name, app_name)
		...
		var result models.ProvisioningProfileResponse
...
func CreateProvisioning_profileTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("get_v0_1_apps_owner_name_app_name_releases_release_id_provisioning_profile",
		mcp.WithDescription("Return information about the provisioning profile. Only available for iOS."),
	...
	return models.Tool{
		Definition: tool,
		Handler:    Provisioning_profileHandler(cfg),
	}
}
