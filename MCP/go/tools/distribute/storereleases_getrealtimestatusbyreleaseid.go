...
func Storereleases_getrealtimestatusbyreleaseidHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	return func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		url := fmt.Sprintf("%s/v0.1/apps/%s/%s/distribution_stores/%s/releases/%s/realtimestatus", cfg.BaseURL, store_name, release_id, owner_name, app_name)
...
func CreateStorereleases_getrealtimestatusbyreleaseidTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("get_v0_1_apps_owner_name_app_name_distribution_stores_store_name_releases_release_id_realtimestatus",
		mcp.WithDescription("Return the Real time Status publishing of release from store."),
	...
	return models.Tool{
		Definition: tool,
		Handler:    Storereleases_getrealtimestatusbyreleaseidHandler(cfg),
	}
}
