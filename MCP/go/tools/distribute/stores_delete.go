...
func Stores_deleteHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
...
func CreateStores_deleteTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("delete_v0_1_apps_owner_name_app_name_distribution_stores_store_name",
		mcp.WithDescription("delete the store based on specific store name."),
	...
	return models.Tool{
		Definition: tool,
		Handler:    Stores_deleteHandler(cfg),
	}
}
