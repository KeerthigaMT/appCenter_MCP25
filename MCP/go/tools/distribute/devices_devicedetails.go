...
func Devices_devicedetailsHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
...
func CreateDevices_devicedetailsTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("get_v0_1_user_devices_device_udid",
		mcp.WithDescription("Returns the device details."),
		mcp.WithString("device_udid", mcp.Required(), mcp.Description("The UDID of the device")),
	)

	return models.Tool{
		Definition: tool,
		Handler:    Devices_devicedetailsHandler(cfg),
	}
}
