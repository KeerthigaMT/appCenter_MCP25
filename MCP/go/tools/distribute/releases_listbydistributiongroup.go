...
func Releases_listbydistributiongroupHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	return func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		url := fmt.Sprintf("%s/v0.1/apps/%s/%s/distribution_groups/%s/releases", cfg.BaseURL, distribution_group_name, owner_name, app_name)
...
func CreateReleases_listbydistributiongroupTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("get_v0_1_apps_owner_name_app_name_distribution_groups_distribution_group_name_releases",
		mcp.WithDescription("Return basic information about distributed releases in a given distribution group."),
	...
	return models.Tool{
		Definition: tool,
		Handler:    Releases_listbydistributiongroupHandler(cfg),
	}
}
