...
func Releases_deletewithdistributiongroupidHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	return func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		url := fmt.Sprintf("%s/v0.1/apps/%s/%s/distribution_groups/%s/releases/%s", cfg.BaseURL, owner_name, app_name, distribution_group_name, release_id)
...
func CreateReleases_deletewithdistributiongroupidTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("delete_v0_1_apps_owner_name_app_name_distribution_groups_distribution_group_name_releases_release_id",
		mcp.WithDescription("Deletes a release with id 'release_id' in a given distribution group."),
		...
		mcp.WithNumber("release_id", mcp.Required(), mcp.Description("The ID identifying the unique release.")),
	)

	return models.Tool{
		Definition: tool,
		Handler:    Releases_deletewithdistributiongroupidHandler(cfg),
	}
}
