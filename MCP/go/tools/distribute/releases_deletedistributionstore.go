...
func Releases_deletedistributionstoreHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	return func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		store_idVal, ok := args["store_id"]
		if !ok {
			return mcp.NewToolResultError("Missing required path parameter: store_id"), nil
		}
		store_id, ok := store_idVal.(string)
		if !ok {
			return mcp.NewToolResultError("Invalid path parameter: store_id"), nil
		}
		...
		url := fmt.Sprintf("%s/v0.1/apps/%s/%s/releases/%s/stores/%s", cfg.BaseURL, release_id, store_id, owner_name, app_name)
...
func CreateReleases_deletedistributionstoreTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("delete_v0_1_apps_owner_name_app_name_releases_release_id_stores_store_id",
		mcp.WithDescription("Delete the given distribution store from the release"),
		mcp.WithNumber("release_id", mcp.Required(), mcp.Description("The ID of the release")),
		mcp.WithString("store_id", mcp.Required(), mcp.Description("The id of the distribution store")),
	...
	return models.Tool{
		Definition: tool,
		Handler:    Releases_deletedistributionstoreHandler(cfg),
	}
}
