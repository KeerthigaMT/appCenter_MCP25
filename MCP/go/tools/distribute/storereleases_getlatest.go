...
func Storereleases_getlatestHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	return func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		url := fmt.Sprintf("%s/v0.1/apps/%s/%s/distribution_stores/%s/latest_release", cfg.BaseURL, store_name, owner_name, app_name)
...
func CreateStorereleases_getlatestTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("get_v0_1_apps_owner_name_app_name_distribution_stores_store_name_latest_release",
		mcp.WithDescription("Returns the latest release published in a store."),
	...
	return models.Tool{
		Definition: tool,
		Handler:    Storereleases_getlatestHandler(cfg),
	}
}
