...
func Missingsymbolgroups_listHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	return func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		url := fmt.Sprintf("%s/v0.1/apps/%s/%s/diagnostics/symbol_groups%s", cfg.BaseURL, owner_name, app_name, queryString)
...
func CreateMissingsymbolgroups_listTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("get_v0_1_apps_owner_name_app_name_diagnostics_symbol_groups",
		mcp.WithDescription("Gets top N (ordered by crash count) of crash groups by missing symbol"),
		mcp.WithNumber("top", mcp.Required(), mcp.Description("top N elements")),
	...
	return models.Tool{
		Definition: tool,
		Handler:    Missingsymbolgroups_listHandler(cfg),
	}
}
