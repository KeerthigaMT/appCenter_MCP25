...
func Symbols_listHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	return func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		url := fmt.Sprintf("%s/v0.1/apps/%s/%s/symbols", cfg.BaseURL, owner_name, app_name)
...
func CreateSymbols_listTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("get_v0_1_apps_owner_name_app_name_symbols",
		mcp.WithDescription("Returns the list of all symbols for the provided application"),
	...
	return models.Tool{
		Definition: tool,
		Handler:    Symbols_listHandler(cfg),
	}
}
