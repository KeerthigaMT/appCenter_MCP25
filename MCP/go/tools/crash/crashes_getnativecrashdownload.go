...
func Crashes_getnativecrashdownloadHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	return func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		url := fmt.Sprintf("%s/v0.1/apps/%s/%s/crash_groups/%s/crashes/%s/native/download", cfg.BaseURL, crash_group_id, crash_id, owner_name, app_name)
...
func CreateCrashes_getnativecrashdownloadTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("get_v0_1_apps_owner_name_app_name_crash_groups_crash_group_id_crashes_crash_id_native_download",
		mcp.WithDescription("Gets the native log of a specific crash as a text attachment."),
	...
	return models.Tool{
		Definition: tool,
		Handler:    Crashes_getnativecrashdownloadHandler(cfg),
	}
}
