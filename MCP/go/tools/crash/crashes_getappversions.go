...
func Crashes_getappversionsHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	return func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		url := fmt.Sprintf("%s/v0.1/apps/%s/%s/versions", cfg.BaseURL, owner_name, app_name)
...
func CreateCrashes_getappversionsTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("get_v0_1_apps_owner_name_app_name_versions",
		mcp.WithDescription("Gets a list of application versions."),
	...
	return models.Tool{
		Definition: tool,
		Handler:    Crashes_getappversionsHandler(cfg),
	}
}
