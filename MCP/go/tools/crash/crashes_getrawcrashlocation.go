...
func Crashes_getrawcrashlocationHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	return func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		url := fmt.Sprintf("%s/v0.1/apps/%s/%s/crash_groups/%s/crashes/%s/raw/location", cfg.BaseURL, crash_group_id, crash_id, owner_name, app_name)
...
func CreateCrashes_getrawcrashlocationTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("get_v0_1_apps_owner_name_app_name_crash_groups_crash_group_id_crashes_crash_id_raw_location",
		mcp.WithDescription("Gets the URI location to download json of a specific crash."),
	...
	return models.Tool{
		Definition: tool,
		Handler:    Crashes_getrawcrashlocationHandler(cfg),
	}
}
