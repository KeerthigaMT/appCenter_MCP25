...
func Crashes_getHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	return func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		crash_idVal, ok := args["crash_id"]
		if !ok {
			return mcp.NewToolResultError("Missing required path parameter: crash_id"), nil
		}
		crash_id, ok := crash_idVal.(string)
		if !ok {
			return mcp.NewToolResultError("Invalid path parameter: crash_id"), nil
		}
		...
		if val, ok := args["include_details"]; ok {
			queryParams = append(queryParams, fmt.Sprintf("include_details=%v", val))
		}
		if val, ok := args["include_stacktrace"]; ok {
			queryParams = append(queryParams, fmt.Sprintf("include_stacktrace=%v", val))
		}
		if val, ok := args["grouping_only"]; ok {
			queryParams = append(queryParams, fmt.Sprintf("grouping_only=%v", val))
		}
		queryString := ""
		if len(queryParams) > 0 {
			queryString = "?" + strings.Join(queryParams, "&")
		}
		url := fmt.Sprintf("%s/v0.1/apps/%s/%s/crash_groups/%s/crashes/%s%s", cfg.BaseURL, crash_group_id, crash_id, owner_name, app_name, queryString)
...
func CreateCrashes_getTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("get_v0_1_apps_owner_name_app_name_crash_groups_crash_group_id_crashes_crash_id",
		mcp.WithDescription("Gets a specific crash for an app."),
		mcp.WithString("crash_group_id", mcp.Required(), mcp.Description("id of a specific group")),
		mcp.WithString("crash_id", mcp.Required(), mcp.Description("id of a specific crash")),
		...
		mcp.WithBoolean("include_details", mcp.Description("true if the crash should include in depth crash details")),
		mcp.WithBoolean("include_stacktrace", mcp.Description("true if the crash should include the stacktrace information")),
		mcp.WithBoolean("grouping_only", mcp.Description("true if the stacktrace should be only the relevant thread / exception. Default is false")),
	...
	return models.Tool{
		Definition: tool,
		Handler:    Crashes_getHandler(cfg),
	}
}
