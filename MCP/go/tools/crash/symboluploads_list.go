...
func Symboluploads_listHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	return func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		if val, ok := args["symbol_type"]; ok {
			queryParams = append(queryParams, fmt.Sprintf("symbol_type=%v", val))
		}
		queryString := ""
		if len(queryParams) > 0 {
			queryString = "?" + strings.Join(queryParams, "&")
		}
		url := fmt.Sprintf("%s/v0.1/apps/%s/%s/symbol_uploads%s", cfg.BaseURL, owner_name, app_name, queryString)
...
func CreateSymboluploads_listTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("get_v0_1_apps_owner_name_app_name_symbol_uploads",
		mcp.WithDescription("Gets a list of all uploads for the specified application"),
		...
		mcp.WithString("symbol_type", mcp.Description("The type of symbols")),
	...
	return models.Tool{
		Definition: tool,
		Handler:    Symboluploads_listHandler(cfg),
	}
}
