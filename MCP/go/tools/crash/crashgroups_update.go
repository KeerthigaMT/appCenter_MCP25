...
func Crashgroups_updateHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	return func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		url := fmt.Sprintf("%s/v0.1/apps/%s/%s/crash_groups/%s", cfg.BaseURL, crash_group_id, owner_name, app_name)
...
func CreateCrashgroups_updateTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("patch_v0_1_apps_owner_name_app_name_crash_groups_crash_group_id",
		mcp.WithDescription("Updates a group."),
	...
	return models.Tool{
		Definition: tool,
		Handler:    Crashgroups_updateHandler(cfg),
	}
}
