...
func Missingsymbolgroups_infoHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	return func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		url := fmt.Sprintf("%s/v0.1/apps/%s/%s/diagnostics/symbol_groups_info", cfg.BaseURL, owner_name, app_name)
...
func CreateMissingsymbolgroups_infoTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("get_v0_1_apps_owner_name_app_name_diagnostics_symbol_groups_info",
		mcp.WithDescription("Gets application level statistics for all missing symbol groups"),
	...
	return models.Tool{
		Definition: tool,
		Handler:    Missingsymbolgroups_infoHandler(cfg),
	}
}
