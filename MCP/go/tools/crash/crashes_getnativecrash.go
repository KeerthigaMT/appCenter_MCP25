...
func Crashes_getnativecrashHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	return func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		url := fmt.Sprintf("%s/v0.1/apps/%s/%s/crash_groups/%s/crashes/%s/native", cfg.BaseURL, crash_group_id, crash_id, owner_name, app_name)
...
func CreateCrashes_getnativecrashTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("get_v0_1_apps_owner_name_app_name_crash_groups_crash_group_id_crashes_crash_id_native",
		mcp.WithDescription("Gets the native log of a specific crash."),
	...
	return models.Tool{
		Definition: tool,
		Handler:    Crashes_getnativecrashHandler(cfg),
	}
}
