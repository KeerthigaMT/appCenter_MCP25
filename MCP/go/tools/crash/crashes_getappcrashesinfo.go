...
func Crashes_getappcrashesinfoHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	return func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		url := fmt.Sprintf("%s/v0.1/apps/%s/%s/crashes_info", cfg.BaseURL, owner_name, app_name)
...
func CreateCrashes_getappcrashesinfoTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("get_v0_1_apps_owner_name_app_name_crashes_info",
		mcp.WithDescription("Gets whether the application has any crashes."),
	...
	return models.Tool{
		Definition: tool,
		Handler:    Crashes_getappcrashesinfoHandler(cfg),
	}
}
