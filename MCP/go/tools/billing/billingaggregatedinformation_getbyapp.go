...
func Billingaggregatedinformation_getbyappHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	return func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		url := fmt.Sprintf("%s/v0.1/apps/%s/%s/billing/aggregated%s", cfg.BaseURL, owner_name, app_name, queryString)
...
func CreateBillingaggregatedinformation_getbyappTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("get_v0_1_apps_owner_name_app_name_billing_aggregated",
		mcp.WithDescription("Aggregated Billing Information for owner of a given app."),
	...
	return models.Tool{
		Definition: tool,
		Handler:    Billingaggregatedinformation_getbyappHandler(cfg),
	}
}
