...
func Codepushdeploymentmetrics_getHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	return func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		url := fmt.Sprintf("%s/v0.1/apps/%s/%s/deployments/%s/metrics", cfg.BaseURL, deployment_name, owner_name, app_name)
...
func CreateCodepushdeploymentmetrics_getTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("get_v0_1_apps_owner_name_app_name_deployments_deployment_name_metrics",
		mcp.WithDescription("Gets all releases metrics for specified Deployment"),
	...
	return models.Tool{
		Definition: tool,
		Handler:    Codepushdeploymentmetrics_getHandler(cfg),
	}
}
