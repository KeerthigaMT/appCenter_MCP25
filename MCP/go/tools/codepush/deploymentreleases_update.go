...
func Deploymentreleases_updateHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	return func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		release_labelVal, ok := args["release_label"]
		if !ok {
			return mcp.NewToolResultError("Missing required path parameter: release_label"), nil
		}
		release_label, ok := release_labelVal.(string)
		if !ok {
			return mcp.NewToolResultError("Invalid path parameter: release_label"), nil
		}
		...
		url := fmt.Sprintf("%s/v0.1/apps/%s/%s/deployments/%s/releases/%s", cfg.BaseURL, deployment_name, release_label, owner_name, app_name)
...
func CreateDeploymentreleases_updateTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("patch_v0_1_apps_owner_name_app_name_deployments_deployment_name_releases_release_label",
		mcp.WithDescription("Modifies a CodePush release metadata under the given Deployment"),
		mcp.WithString("deployment_name", mcp.Required(), mcp.Description("deployment name")),
		mcp.WithString("release_label", mcp.Required(), mcp.Description("release label")),
	...
	return models.Tool{
		Definition: tool,
		Handler:    Deploymentreleases_updateHandler(cfg),
	}
}
