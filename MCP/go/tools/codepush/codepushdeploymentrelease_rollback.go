...
func Codepushdeploymentrelease_rollbackHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	return func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		url := fmt.Sprintf("%s/v0.1/apps/%s/%s/deployments/%s/rollback_release", cfg.BaseURL, deployment_name, owner_name, app_name)
...
func CreateCodepushdeploymentrelease_rollbackTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("post_v0_1_apps_owner_name_app_name_deployments_deployment_name_rollback_release",
		mcp.WithDescription("Rollback the latest or a specific release for an app deployment"),
	...
	return models.Tool{
		Definition: tool,
		Handler:    Codepushdeploymentrelease_rollbackHandler(cfg),
	}
}
