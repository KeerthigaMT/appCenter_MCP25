...
func Codepushdeployments_updateHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	return func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		url := fmt.Sprintf("%s/v0.1/apps/%s/%s/deployments/%s", cfg.BaseURL, deployment_name, owner_name, app_name)
...
func CreateCodepushdeployments_updateTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("patch_v0_1_apps_owner_name_app_name_deployments_deployment_name",
		mcp.WithDescription("Modifies a CodePush Deployment for the given app"),
	...
	return models.Tool{
		Definition: tool,
		Handler:    Codepushdeployments_updateHandler(cfg),
	}
}
