...
func Codepushdeployments_createHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	return func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		url := fmt.Sprintf("%s/v0.1/apps/%s/%s/deployments", cfg.BaseURL, owner_name, app_name)
...
func CreateCodepushdeployments_createTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("post_v0_1_apps_owner_name_app_name_deployments",
		mcp.WithDescription("Creates a CodePush Deployment for the given app"),
		...
		mcp.WithString("latest_release", mcp.Description("")),
		mcp.WithString("name", mcp.Required(), mcp.Description("")),
	)

	return models.Tool{
		Definition: tool,
		Handler:    Codepushdeployments_createHandler(cfg),
	}
}
