...
func Codepushdeploymentupload_createHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	return func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		url := fmt.Sprintf("%s/v0.1/apps/%s/%s/deployments/%s/uploads", cfg.BaseURL, deployment_name, owner_name, app_name)
...
func CreateCodepushdeploymentupload_createTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("post_v0_1_apps_owner_name_app_name_deployments_deployment_name_uploads",
		mcp.WithDescription("Create a new CodePush release upload for the specified deployment"),
	...
	return models.Tool{
		Definition: tool,
		Handler:    Codepushdeploymentupload_createHandler(cfg),
	}
}
