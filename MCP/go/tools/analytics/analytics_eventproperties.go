...
func Analytics_eventpropertiesHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	return func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		url := fmt.Sprintf("%s/v0.1/apps/%s/%s/analytics/events/%s/properties", cfg.BaseURL, event_name, owner_name, app_name)
...
func CreateAnalytics_eventpropertiesTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("get_v0_1_apps_owner_name_app_name_analytics_events_event_name_properties",
		mcp.WithDescription("Event properties."),
	...
	return models.Tool{
		Definition: tool,
		Handler:    Analytics_eventpropertiesHandler(cfg),
	}
}
