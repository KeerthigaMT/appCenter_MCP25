...
func Analytics_listdevicepropertyvaluesHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	return func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		url := fmt.Sprintf("%s/v0.1/apps/%s/%s/analytics/audiences/metadata/device_properties/%s/values%s", cfg.BaseURL, property_name, owner_name, app_name, queryString)
...
func CreateAnalytics_listdevicepropertyvaluesTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("get_v0_1_apps_owner_name_app_name_analytics_audiences_metadata_device_properties_property_name_values",
		mcp.WithDescription("Get list of device property values."),
		mcp.WithString("property_name", mcp.Required(), mcp.Description("Device property")),
		mcp.WithString("contains", mcp.Description("Contains string")),
	...
	return models.Tool{
		Definition: tool,
		Handler:    Analytics_listdevicepropertyvaluesHandler(cfg),
	}
}
