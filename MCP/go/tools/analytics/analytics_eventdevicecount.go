...
func Analytics_eventdevicecountHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	return func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		url := fmt.Sprintf("%s/v0.1/apps/%s/%s/analytics/events/%s/device_count%s", cfg.BaseURL, event_name, owner_name, app_name, queryString)
...
func CreateAnalytics_eventdevicecountTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("get_v0_1_apps_owner_name_app_name_analytics_events_event_name_device_count",
		mcp.WithDescription("Count of devices for an event by interval in the time range."),
	...
	return models.Tool{
		Definition: tool,
		Handler:    Analytics_eventdevicecountHandler(cfg),
	}
}
