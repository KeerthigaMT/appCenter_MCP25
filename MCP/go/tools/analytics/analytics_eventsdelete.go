...
func Analytics_eventsdeleteHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	return func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		url := fmt.Sprintf("%s/v0.1/apps/%s/%s/analytics/events/%s", cfg.BaseURL, event_name, owner_name, app_name)
...
func CreateAnalytics_eventsdeleteTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("delete_v0_1_apps_owner_name_app_name_analytics_events_event_name",
	...
	return models.Tool{
		Definition: tool,
		Handler:    Analytics_eventsdeleteHandler(cfg),
	}
}
