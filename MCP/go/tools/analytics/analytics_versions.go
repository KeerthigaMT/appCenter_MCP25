...
func Analytics_versionsHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	return func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		url := fmt.Sprintf("%s/v0.1/apps/%s/%s/analytics/versions%s", cfg.BaseURL, owner_name, app_name, queryString)
...
func CreateAnalytics_versionsTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("get_v0_1_apps_owner_name_app_name_analytics_versions",
		mcp.WithDescription("Count of active versions in the time range ordered by version."),
	...
	return models.Tool{
		Definition: tool,
		Handler:    Analytics_versionsHandler(cfg),
	}
}
