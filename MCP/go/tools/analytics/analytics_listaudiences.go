...
func Analytics_listaudiencesHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	return func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		url := fmt.Sprintf("%s/v0.1/apps/%s/%s/analytics/audiences%s", cfg.BaseURL, owner_name, app_name, queryString)
...
func CreateAnalytics_listaudiencesTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("get_v0_1_apps_owner_name_app_name_analytics_audiences",
		mcp.WithDescription("Get list of audiences."),
		mcp.WithBoolean("include_disabled", mcp.Description("Include disabled audience definitions")),
	...
	return models.Tool{
		Definition: tool,
		Handler:    Analytics_listaudiencesHandler(cfg),
	}
}
