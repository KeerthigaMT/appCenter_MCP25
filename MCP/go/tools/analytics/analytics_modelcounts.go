...
func Analytics_modelcountsHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	return func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		url := fmt.Sprintf("%s/v0.1/apps/%s/%s/analytics/models%s", cfg.BaseURL, owner_name, app_name, queryString)
...
func CreateAnalytics_modelcountsTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("get_v0_1_apps_owner_name_app_name_analytics_models",
		mcp.WithDescription("Models in the time range."),
	...
	return models.Tool{
		Definition: tool,
		Handler:    Analytics_modelcountsHandler(cfg),
	}
}
