...
func Analytics_crashfreedevicepercentagesHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	return func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		url := fmt.Sprintf("%s/v0.1/apps/%s/%s/analytics/crashfree_device_percentages%s", cfg.BaseURL, owner_name, app_name, queryString)
...
func CreateAnalytics_crashfreedevicepercentagesTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("get_v0_1_apps_owner_name_app_name_analytics_crashfree_device_percentages",
		mcp.WithDescription("Percentage of crash-free device by day in the time range based on the selected versions. Api will return -1 if crash devices is greater than active devices."),
	...
	return models.Tool{
		Definition: tool,
		Handler:    Analytics_crashfreedevicepercentagesHandler(cfg),
	}
}
