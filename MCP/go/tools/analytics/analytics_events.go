...
func Analytics_eventsHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	return func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		if val, ok := args["event_name"]; ok {
			queryParams = append(queryParams, fmt.Sprintf("event_name=%v", val))
		}
		...
		url := fmt.Sprintf("%s/v0.1/apps/%s/%s/analytics/events%s", cfg.BaseURL, owner_name, app_name, queryString)
...
func CreateAnalytics_eventsTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("get_v0_1_apps_owner_name_app_name_analytics_events",
		mcp.WithDescription("Count of active events in the time range ordered by event."),
		...
		mcp.WithArray("event_name", mcp.Description("To select the specific events.")),
		mcp.WithNumber("$top", mcp.Description("The maximum number of results to return. (0 will fetch all results)")),
		...
		mcp.WithString("$orderby", mcp.Description("controls the sorting order and sorting based on which column")),
	...
	return models.Tool{
		Definition: tool,
		Handler:    Analytics_eventsHandler(cfg),
	}
}
