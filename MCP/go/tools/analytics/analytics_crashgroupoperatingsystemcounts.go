...
func Analytics_crashgroupoperatingsystemcountsHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	return func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		url := fmt.Sprintf("%s/v0.1/apps/%s/%s/analytics/crash_groups/%s/operating_systems%s", cfg.BaseURL, crash_group_id, owner_name, app_name, queryString)
...
func CreateAnalytics_crashgroupoperatingsystemcountsTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("get_v0_1_apps_owner_name_app_name_analytics_crash_groups_crash_group_id_operating_systems",
		mcp.WithDescription("Available for UWP apps only."),
		mcp.WithString("crash_group_id", mcp.Required(), mcp.Description("The id of the crash group.")),
	...
	return models.Tool{
		Definition: tool,
		Handler:    Analytics_crashgroupoperatingsystemcountsHandler(cfg),
	}
}
