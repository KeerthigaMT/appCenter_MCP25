...
func Analytics_crashgroupstotalsHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	return func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		url := fmt.Sprintf("%s/v0.1/apps/%s/%s/analytics/crash_groups", cfg.BaseURL, owner_name, app_name)
...
func CreateAnalytics_crashgroupstotalsTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("post_v0_1_apps_owner_name_app_name_analytics_crash_groups",
		mcp.WithDescription("Overall crashes and affected users count of the selected crash groups with selected versions."),
		...
		mcp.WithArray("crash_groups", mcp.Required(), mcp.Description("")),
	)

	return models.Tool{
		Definition: tool,
		Handler:    Analytics_crashgroupstotalsHandler(cfg),
	}
}
