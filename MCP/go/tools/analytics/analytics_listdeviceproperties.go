...
func Analytics_listdevicepropertiesHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	return func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		url := fmt.Sprintf("%s/v0.1/apps/%s/%s/analytics/audiences/metadata/device_properties", cfg.BaseURL, owner_name, app_name)
...
func CreateAnalytics_listdevicepropertiesTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("get_v0_1_apps_owner_name_app_name_analytics_audiences_metadata_device_properties",
		mcp.WithDescription("Get list of device properties."),
	...
	return models.Tool{
		Definition: tool,
		Handler:    Analytics_listdevicepropertiesHandler(cfg),
	}
}
