...
func Analytics_eventpersessioncountHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	return func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		url := fmt.Sprintf("%s/v0.1/apps/%s/%s/analytics/events/%s/count_per_session%s", cfg.BaseURL, event_name, owner_name, app_name, queryString)
...
func CreateAnalytics_eventpersessioncountTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("get_v0_1_apps_owner_name_app_name_analytics_events_event_name_count_per_session",
		mcp.WithDescription("Count of events per session by interval in the time range."),
	...
	return models.Tool{
		Definition: tool,
		Handler:    Analytics_eventpersessioncountHandler(cfg),
	}
}
