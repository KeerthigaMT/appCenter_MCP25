...
func App_blocklogsHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	return func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		url := fmt.Sprintf("%s/v0.1/apps/%s/%s/devices/block_logs", cfg.BaseURL, owner_name, app_name)
...
func CreateApp_blocklogsTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("put_v0_1_apps_owner_name_app_name_devices_block_logs",
...
"),
	...
	return models.Tool{
		Definition: tool,
		Handler:    App_blocklogsHandler(cfg),
	}
}
