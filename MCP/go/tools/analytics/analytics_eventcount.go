...
func Analytics_eventcountHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	return func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		url := fmt.Sprintf("%s/v0.1/apps/%s/%s/analytics/events/%s/event_count%s", cfg.BaseURL, event_name, owner_name, app_name, queryString)
...
func CreateAnalytics_eventcountTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("get_v0_1_apps_owner_name_app_name_analytics_events_event_name_event_count",
		mcp.WithDescription("Count of events by interval in the time range."),
	...
	return models.Tool{
		Definition: tool,
		Handler:    Analytics_eventcountHandler(cfg),
	}
}
