...
func Analytics_distributionreleasecountsHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	return func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		url := fmt.Sprintf("%s/v0.1/apps/%s/%s/analytics/distribution/release_counts", cfg.BaseURL, owner_name, app_name)
...
func CreateAnalytics_distributionreleasecountsTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("post_v0_1_apps_owner_name_app_name_analytics_distribution_release_counts",
		mcp.WithDescription("Count of total downloads for the provided distribution releases."),
		...
		mcp.WithArray("releases", mcp.Required(), mcp.Description("")),
	)

	return models.Tool{
		Definition: tool,
		Handler:    Analytics_distributionreleasecountsHandler(cfg),
	}
}
