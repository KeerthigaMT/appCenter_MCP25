...
func Analytics_eventsdeletelogsHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	return func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		url := fmt.Sprintf("%s/v0.1/apps/%s/%s/analytics/event_logs/%s", cfg.BaseURL, event_name, owner_name, app_name)
...
func CreateAnalytics_eventsdeletelogsTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("delete_v0_1_apps_owner_name_app_name_analytics_event_logs_event_name",
		mcp.WithDescription("Delete the set of Events with the specified event names."),
	...
	return models.Tool{
		Definition: tool,
		Handler:    Analytics_eventsdeletelogsHandler(cfg),
	}
}
