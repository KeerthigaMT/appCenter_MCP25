...
func Errors_countsperdayHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	return func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		url := fmt.Sprintf("%s/v0.1/apps/%s/%s/errors/errorCountsPerDay%s", cfg.BaseURL, owner_name, app_name, queryString)
...
func CreateErrors_countsperdayTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("get_v0_1_apps_owner_name_app_name_errors_errorCountsPerDay",
		mcp.WithDescription("Count of crashes or errors by day in the time range based the selected versions. If SingleErrorTypeParameter is not provided, defaults to handlederror."),
	...
	return models.Tool{
		Definition: tool,
		Handler:    Errors_countsperdayHandler(cfg),
	}
}
