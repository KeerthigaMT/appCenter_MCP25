...
func Errors_errordownloadHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	return func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		url := fmt.Sprintf("%s/v0.1/apps/%s/%s/errors/errorGroups/%s/errors/%s/download%s", cfg.BaseURL, errorGroupId, errorId, owner_name, app_name, queryString)
...
func CreateErrors_errordownloadTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("get_v0_1_apps_owner_name_app_name_errors_errorGroups_errorGroupId_errors_errorId_download",
		mcp.WithDescription("Download details for a specific error."),
		...
		mcp.WithString("format", mcp.Description("the format of the crash log")),
	...
	return models.Tool{
		Definition: tool,
		Handler:    Errors_errordownloadHandler(cfg),
	}
}
