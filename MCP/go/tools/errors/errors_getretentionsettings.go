...
func Errors_getretentionsettingsHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	return func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		url := fmt.Sprintf("%s/v0.1/apps/%s/%s/errors/retention_settings", cfg.BaseURL, owner_name, app_name)
...
func CreateErrors_getretentionsettingsTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("get_v0_1_apps_owner_name_app_name_errors_retention_settings",
		mcp.WithDescription("gets the retention settings in days"),
	...
	return models.Tool{
		Definition: tool,
		Handler:    Errors_getretentionsettingsHandler(cfg),
	}
}
