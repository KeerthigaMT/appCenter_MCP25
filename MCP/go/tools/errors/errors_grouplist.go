...
func Errors_grouplistHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	return func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		if val, ok := args["groupState"]; ok {
			queryParams = append(queryParams, fmt.Sprintf("groupState=%v", val))
		}
		...
		url := fmt.Sprintf("%s/v0.1/apps/%s/%s/errors/errorGroups%s", cfg.BaseURL, owner_name, app_name, queryString)
...
func CreateErrors_grouplistTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("get_v0_1_apps_owner_name_app_name_errors_errorGroups",
		mcp.WithDescription("List of error groups"),
		mcp.WithString("version", mcp.Description("")),
		mcp.WithString("app_build", mcp.Description("app build")),
		mcp.WithString("groupState", mcp.Description("")),
	...
	return models.Tool{
		Definition: tool,
		Handler:    Errors_grouplistHandler(cfg),
	}
}
