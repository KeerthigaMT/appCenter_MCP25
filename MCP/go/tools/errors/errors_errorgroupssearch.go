...
func Errors_errorgroupssearchHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	return func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		url := fmt.Sprintf("%s/v0.1/apps/%s/%s/errors/errorGroups/search%s", cfg.BaseURL, owner_name, app_name, queryString)
...
func CreateErrors_errorgroupssearchTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("get_v0_1_apps_owner_name_app_name_errors_errorGroups_search",
		mcp.WithDescription("Error groups list based on search parameters"),
		mcp.WithString("filter", mcp.Description("A filter as specified in OData notation")),
		mcp.WithString("q", mcp.Description("A query string")),
		mcp.WithString("order", mcp.Description("It controls the order of sorting")),
		mcp.WithString("sort", mcp.Description("It controls the sort based on specified field")),
		mcp.WithNumber("$top", mcp.Description("The maximum number of results to return")),
	...
	return models.Tool{
		Definition: tool,
		Handler:    Errors_errorgroupssearchHandler(cfg),
	}
}
