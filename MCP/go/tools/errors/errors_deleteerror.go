...
func Errors_deleteerrorHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	return func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		url := fmt.Sprintf("%s/v0.1/apps/%s/%s/errors/errorGroups/%s/errors/%s", cfg.BaseURL, errorGroupId, errorId, owner_name, app_name)
...
func CreateErrors_deleteerrorTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("delete_v0_1_apps_owner_name_app_name_errors_errorGroups_errorGroupId_errors_errorId",
		mcp.WithDescription("Delete a specific error and related attachments and blobs for an app. Searchable data will not be deleted immediately and may take up to 30 days."),
	...
	return models.Tool{
		Definition: tool,
		Handler:    Errors_deleteerrorHandler(cfg),
	}
}
