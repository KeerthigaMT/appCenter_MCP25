...
func Errors_groupcountsperdayHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	return func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		url := fmt.Sprintf("%s/v0.1/apps/%s/%s/errors/errorGroups/%s/errorCountsPerDay%s", cfg.BaseURL, errorGroupId, owner_name, app_name, queryString)
...
func CreateErrors_groupcountsperdayTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("get_v0_1_apps_owner_name_app_name_errors_errorGroups_errorGroupId_errorCountsPerDay",
		mcp.WithDescription("Count of errors by day in the time range of the selected error group with selected version"),
	...
	return models.Tool{
		Definition: tool,
		Handler:    Errors_groupcountsperdayHandler(cfg),
	}
}
