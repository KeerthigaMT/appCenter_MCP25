...
func Errors_errorsearchHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	return func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		url := fmt.Sprintf("%s/v0.1/apps/%s/%s/errors/search%s", cfg.BaseURL, owner_name, app_name, queryString)
...
func CreateErrors_errorsearchTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("get_v0_1_apps_owner_name_app_name_errors_search",
		mcp.WithDescription("Errors list based on search parameters"),
	...
	return models.Tool{
		Definition: tool,
		Handler:    Errors_errorsearchHandler(cfg),
	}
}
