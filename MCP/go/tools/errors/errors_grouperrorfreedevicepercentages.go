...
func Errors_grouperrorfreedevicepercentagesHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	return func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		url := fmt.Sprintf("%s/v0.1/apps/%s/%s/errors/errorGroups/%s/errorfreeDevicePercentages%s", cfg.BaseURL, errorGroupId, owner_name, app_name, queryString)
...
func CreateErrors_grouperrorfreedevicepercentagesTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("get_v0_1_apps_owner_name_app_name_errors_errorGroups_errorGroupId_errorfreeDevicePercentages",
		mcp.WithDescription("Percentage of error-free devices by day in the time range. Api will return -1 if crash devices is greater than active devices"),
	...
	return models.Tool{
		Definition: tool,
		Handler:    Errors_grouperrorfreedevicepercentagesHandler(cfg),
	}
}
