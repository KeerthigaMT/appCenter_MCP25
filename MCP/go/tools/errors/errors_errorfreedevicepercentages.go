...
func Errors_errorfreedevicepercentagesHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	return func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		url := fmt.Sprintf("%s/v0.1/apps/%s/%s/errors/errorfreeDevicePercentages%s", cfg.BaseURL, owner_name, app_name, queryString)
...
func CreateErrors_errorfreedevicepercentagesTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("get_v0_1_apps_owner_name_app_name_errors_errorfreeDevicePercentages",
		mcp.WithDescription("Percentage of error-free devices by day in the time range based on the selected versions. If SingleErrorTypeParameter is not provided, defaults to handlederror. API will return -1 if crash devices is greater than active devices"),
		...
		mcp.WithArray("versions", mcp.Description("")),
		mcp.WithString("app_build", mcp.Description("app build")),
		mcp.WithString("errorType", mcp.Description("Type of error (handled vs unhandled), excluding All")),
	...
	return models.Tool{
		Definition: tool,
		Handler:    Errors_errorfreedevicepercentagesHandler(cfg),
	}
}
