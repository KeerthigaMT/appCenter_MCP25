...
func Errors_grouperrorstacktraceHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	return func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		url := fmt.Sprintf("%s/v0.1/apps/%s/%s/errors/errorGroups/%s/stacktrace", cfg.BaseURL, errorGroupId, owner_name, app_name)
...
func CreateErrors_grouperrorstacktraceTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("get_v0_1_apps_owner_name_app_name_errors_errorGroups_errorGroupId_stacktrace",
		mcp.WithDescription("Gets the stack trace for the error group."),
	...
	return models.Tool{
		Definition: tool,
		Handler:    Errors_grouperrorstacktraceHandler(cfg),
	}
}
