...
func Errors_updatestateHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	return func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		url := fmt.Sprintf("%s/v0.1/apps/%s/%s/errors/errorGroups/%s", cfg.BaseURL, errorGroupId, owner_name, app_name)
...
func CreateErrors_updatestateTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("patch_v0_1_apps_owner_name_app_name_errors_errorGroups_errorGroupId",
		mcp.WithDescription("Update error group state"),
		...
		mcp.WithString("annotation", mcp.Description("")),
		mcp.WithString("state", mcp.Required(), mcp.Description("")),
	)

	return models.Tool{
		Definition: tool,
		Handler:    Errors_updatestateHandler(cfg),
	}
}
