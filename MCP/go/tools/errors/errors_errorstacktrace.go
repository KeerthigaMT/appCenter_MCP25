...
func Errors_errorstacktraceHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	return func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		url := fmt.Sprintf("%s/v0.1/apps/%s/%s/errors/errorGroups/%s/errors/%s/stacktrace", cfg.BaseURL, errorGroupId, errorId, owner_name, app_name)
		...
		var result models.DiagnosticsStackTrace
...
func CreateErrors_errorstacktraceTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("get_v0_1_apps_owner_name_app_name_errors_errorGroups_errorGroupId_errors_errorId_stacktrace",
		mcp.WithDescription("Error Stacktrace details."),
	...
	return models.Tool{
		Definition: tool,
		Handler:    Errors_errorstacktraceHandler(cfg),
	}
}
