...
func Teams_removeappHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
...
func CreateTeams_removeappTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("delete_v0_1_orgs_org_name_teams_team_name_apps_app_name",
		mcp.WithDescription("Removes an app from a team"),
	...
	return models.Tool{
		Definition: tool,
		Handler:    Teams_removeappHandler(cfg),
	}
}
