...
func Teams_updateHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
...
func CreateTeams_updateTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("patch_v0_1_orgs_org_name_teams_team_name",
		mcp.WithDescription("Updates a single team"),
		...
		mcp.WithString("display_name", mcp.Required(), mcp.Description("Input parameter: The new display name of the team")),
	)

	return models.Tool{
		Definition: tool,
		Handler:    Teams_updateHandler(cfg),
	}
}
