...
func Teams_removeuserHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	return func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		url := fmt.Sprintf("%s/v0.1/orgs/%s/teams/%s/users/%s", cfg.BaseURL, org_name, team_name, user_name)
...
func CreateTeams_removeuserTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("delete_v0_1_orgs_org_name_teams_team_name_users_user_name",
		mcp.WithDescription("Removes a user from a team that is owned by an organization"),
	...
	return models.Tool{
		Definition: tool,
		Handler:    Teams_removeuserHandler(cfg),
	}
}
