...
func Appinvitations_createHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	return func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		url := fmt.Sprintf("%s/v0.1/apps/%s/%s/invitations", cfg.BaseURL, owner_name, app_name)
...
func CreateAppinvitations_createTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("post_v0_1_apps_owner_name_app_name_invitations",
		...
		mcp.WithString("role", mcp.Description("Input parameter: The user's role")),
		mcp.WithString("user_email", mcp.Required(), mcp.Description("Input parameter: The user's email address")),
	)

	return models.Tool{
		Definition: tool,
		Handler:    Appinvitations_createHandler(cfg),
	}
}
