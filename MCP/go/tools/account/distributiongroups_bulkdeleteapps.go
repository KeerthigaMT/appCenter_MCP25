...
func Distributiongroups_bulkdeleteappsHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	return func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		url := fmt.Sprintf("%s/v0.1/orgs/%s/distribution_groups/%s/apps/bulk_delete", cfg.BaseURL, org_name, distribution_group_name)
...
func CreateDistributiongroups_bulkdeleteappsTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("post_v0_1_orgs_org_name_distribution_groups_distribution_group_name_apps_bulk_delete",
		mcp.WithDescription("Delete apps from distribution group in an org"),
		...
		mcp.WithArray("apps", mcp.Description("Input parameter: The list of apps to delete from the distribution group")),
	)

	return models.Tool{
		Definition: tool,
		Handler:    Distributiongroups_bulkdeleteappsHandler(cfg),
	}
}
