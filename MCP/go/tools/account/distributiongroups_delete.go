...
func Distributiongroups_deleteHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	return func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		url := fmt.Sprintf("%s/v0.1/apps/%s/%s/distribution_groups/%s", cfg.BaseURL, app_name, owner_name, distribution_group_name)
...
func CreateDistributiongroups_deleteTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("delete_v0_1_apps_owner_name_app_name_distribution_groups_distribution_group_name",
		mcp.WithDescription("Deletes a distribution group"),
	...
	return models.Tool{
		Definition: tool,
		Handler:    Distributiongroups_deleteHandler(cfg),
	}
}
