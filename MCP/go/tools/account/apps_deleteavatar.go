...
func Apps_deleteavatarHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	return func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		url := fmt.Sprintf("%s/v0.1/apps/%s/%s/avatar", cfg.BaseURL, owner_name, app_name)
...
func CreateApps_deleteavatarTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("delete_v0_1_apps_owner_name_app_name_avatar",
		mcp.WithDescription("Deletes the uploaded app avatar"),
	...
	return models.Tool{
		Definition: tool,
		Handler:    Apps_deleteavatarHandler(cfg),
	}
}
