...
func Distributiongroups_getfororgHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
...
func CreateDistributiongroups_getfororgTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("get_v0_1_orgs_org_name_distribution_groups_distribution_group_name",
		mcp.WithDescription("Returns a single distribution group in org for a given distribution group name"),
	...
	return models.Tool{
		Definition: tool,
		Handler:    Distributiongroups_getfororgHandler(cfg),
	}
}
