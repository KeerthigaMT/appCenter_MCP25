...
func Distributiongroups_addappsHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	return func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		url := fmt.Sprintf("%s/v0.1/orgs/%s/distribution_groups/%s/apps", cfg.BaseURL, org_name, distribution_group_name)
...
func CreateDistributiongroups_addappsTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("post_v0_1_orgs_org_name_distribution_groups_distribution_group_name_apps",
		mcp.WithDescription("Add apps to distribution group in an org"),
		...
		mcp.WithArray("apps", mcp.Description("Input parameter: The list of apps to add to distribution group")),
	)

	return models.Tool{
		Definition: tool,
		Handler:    Distributiongroups_addappsHandler(cfg),
	}
}
