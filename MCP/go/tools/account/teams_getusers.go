...
func Teams_getusersHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
...
func CreateTeams_getusersTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("get_v0_1_orgs_org_name_teams_team_name_users",
		mcp.WithDescription("Returns the users of a team which is owned by an organization"),
	...
	return models.Tool{
		Definition: tool,
		Handler:    Teams_getusersHandler(cfg),
	}
}
