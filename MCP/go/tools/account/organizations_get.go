...
func Organizations_getHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
...
func CreateOrganizations_getTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("get_v0_1_orgs_org_name",
		mcp.WithDescription("Returns the details of a single organization"),
		mcp.WithString("org_name", mcp.Required(), mcp.Description("The organization's name")),
	)

	return models.Tool{
		Definition: tool,
		Handler:    Organizations_getHandler(cfg),
	}
}
