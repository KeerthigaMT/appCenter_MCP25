...
func Users_getfororgHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	return func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		url := fmt.Sprintf("%s/v0.1/orgs/%s/users/%s", cfg.BaseURL, org_name, user_name)
...
func CreateUsers_getfororgTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("get_v0_1_orgs_org_name_users_user_name",
		mcp.WithDescription("Get a user information from an organization by name - if there is explicit permission return it, if not if not return highest implicit permission"),
	...
	return models.Tool{
		Definition: tool,
		Handler:    Users_getfororgHandler(cfg),
	}
}
