...
func Organizations_deleteHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
...
func CreateOrganizations_deleteTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("delete_v0_1_orgs_org_name",
		mcp.WithDescription("Deletes a single organization"),
		mcp.WithString("org_name", mcp.Required(), mcp.Description("The organization's name")),
	)

	return models.Tool{
		Definition: tool,
		Handler:    Organizations_deleteHandler(cfg),
	}
}
