...
func Users_removefromorgHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
...
func CreateUsers_removefromorgTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("delete_v0_1_orgs_org_name_users_user_name",
		mcp.WithDescription("Removes a user from an organization."),
	...
	return models.Tool{
		Definition: tool,
		Handler:    Users_removefromorgHandler(cfg),
	}
}
