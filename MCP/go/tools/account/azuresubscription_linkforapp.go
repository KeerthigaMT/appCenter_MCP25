...
func Azuresubscription_linkforappHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	return func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		url := fmt.Sprintf("%s/v0.1/apps/%s/%s/azure_subscriptions", cfg.BaseURL, owner_name, app_name)
...
func CreateAzuresubscription_linkforappTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("post_v0_1_apps_owner_name_app_name_azure_subscriptions",
		mcp.WithDescription("Link azure subscription to an app"),
		...
		mcp.WithString("subscription_id", mcp.Required(), mcp.Description("Input parameter: The azure subscription id")),
	)

	return models.Tool{
		Definition: tool,
		Handler:    Azuresubscription_linkforappHandler(cfg),
	}
}
