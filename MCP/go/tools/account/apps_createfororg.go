...
func Apps_createfororgHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	return func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		url := fmt.Sprintf("%s/v0.1/orgs/%s/apps", cfg.BaseURL, org_name)
...
func CreateApps_createfororgTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("post_v0_1_orgs_org_name_apps",
		mcp.WithDescription("Creates a new app for the organization and returns it to the caller"),
	...
	return models.Tool{
		Definition: tool,
		Handler:    Apps_createfororgHandler(cfg),
	}
}
