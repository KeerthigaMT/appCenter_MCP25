...
func Distributiongroups_bulkdeleteusersHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	return func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		url := fmt.Sprintf("%s/v0.1/orgs/%s/distribution_groups/%s/members/bulk_delete", cfg.BaseURL, org_name, distribution_group_name)
...
func CreateDistributiongroups_bulkdeleteusersTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("post_v0_1_orgs_org_name_distribution_groups_distribution_group_name_members_bulk_delete",
		mcp.WithDescription("Delete testers from distribution group in an org"),
	...
	return models.Tool{
		Definition: tool,
		Handler:    Distributiongroups_bulkdeleteusersHandler(cfg),
	}
}
