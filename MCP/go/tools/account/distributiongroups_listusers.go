...
func Distributiongroups_listusersHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	return func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		if val, ok := args["exclude_pending_invitations"]; ok {
			queryParams = append(queryParams, fmt.Sprintf("exclude_pending_invitations=%v", val))
		}
		queryString := ""
		if len(queryParams) > 0 {
			queryString = "?" + strings.Join(queryParams, "&")
		}
		url := fmt.Sprintf("%s/v0.1/apps/%s/%s/distribution_groups/%s/members%s", cfg.BaseURL, owner_name, app_name, distribution_group_name, queryString)
...
func CreateDistributiongroups_listusersTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("get_v0_1_apps_owner_name_app_name_distribution_groups_distribution_group_name_members",
		mcp.WithDescription("Returns a list of member details in the distribution group specified"),
		...
		mcp.WithBoolean("exclude_pending_invitations", mcp.Description("Whether to exclude pending invitations in the response")),
	)

	return models.Tool{
		Definition: tool,
		Handler:    Distributiongroups_listusersHandler(cfg),
	}
}
