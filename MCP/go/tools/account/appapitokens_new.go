...
func Appapitokens_newHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	return func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		url := fmt.Sprintf("%s/v0.1/apps/%s/%s/api_tokens", cfg.BaseURL, owner_name, app_name)
...
func CreateAppapitokens_newTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("post_v0_1_apps_owner_name_app_name_api_tokens",
		mcp.WithDescription("Creates a new App API token"),
		...
		mcp.WithString("description", mcp.Description("Input parameter: The description of the token")),
		mcp.WithArray("scope", mcp.Description("Input parameter: The scope for this token.")),
	)

	return models.Tool{
		Definition: tool,
		Handler:    Appapitokens_newHandler(cfg),
	}
}
