...
func Apps_transfertoorgHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	return func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		url := fmt.Sprintf("%s/v0.1/apps/%s/%s/transfer_to_org", cfg.BaseURL, owner_name, app_name)
...
func CreateApps_transfertoorgTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("post_v0_1_apps_owner_name_app_name_transfer_to_org",
		mcp.WithDescription("Transfers ownership of an app to a new organization"),
	...
	return models.Tool{
		Definition: tool,
		Handler:    Apps_transfertoorgHandler(cfg),
	}
}
