...
func Users_updateorgroleHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
...
func CreateUsers_updateorgroleTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("patch_v0_1_orgs_org_name_users_user_name",
		mcp.WithDescription("Updates the given organization user"),
	...
	return models.Tool{
		Definition: tool,
		Handler:    Users_updateorgroleHandler(cfg),
	}
}
