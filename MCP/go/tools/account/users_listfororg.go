...
func Users_listfororgHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	return func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		url := fmt.Sprintf("%s/v0.1/orgs/%s/users", cfg.BaseURL, org_name)
...
func CreateUsers_listfororgTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("get_v0_1_orgs_org_name_users",
		mcp.WithDescription("Returns a list of users that belong to an organization"),
		mcp.WithString("org_name", mcp.Required(), mcp.Description("The organization's name")),
	)

	return models.Tool{
		Definition: tool,
		Handler:    Users_listfororgHandler(cfg),
	}
}
