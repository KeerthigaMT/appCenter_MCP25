...
func Distributiongroups_removeuserHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	return func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		url := fmt.Sprintf("%s/v0.1/apps/%s/%s/distribution_groups/%s/members/bulk_delete", cfg.BaseURL, owner_name, app_name, distribution_group_name)
...
func CreateDistributiongroups_removeuserTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("post_v0_1_apps_owner_name_app_name_distribution_groups_distribution_group_name_members_bulk_delete",
		mcp.WithDescription("Remove the users from the distribution group"),
	...
	return models.Tool{
		Definition: tool,
		Handler:    Distributiongroups_removeuserHandler(cfg),
	}
}
