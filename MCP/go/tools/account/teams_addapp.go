...
func Teams_addappHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	return func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		url := fmt.Sprintf("%s/v0.1/orgs/%s/teams/%s/apps", cfg.BaseURL, org_name, team_name)
...
func CreateTeams_addappTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("post_v0_1_orgs_org_name_teams_team_name_apps",
		mcp.WithDescription("Adds an app to a team"),
		...
		mcp.WithString("name", mcp.Required(), mcp.Description("Input parameter: The name of the app to be added to the distribution group")),
	)

	return models.Tool{
		Definition: tool,
		Handler:    Teams_addappHandler(cfg),
	}
}
