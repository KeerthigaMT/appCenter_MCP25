...
func Distributiongroups_patchfororgHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
...
func CreateDistributiongroups_patchfororgTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("patch_v0_1_orgs_org_name_distribution_groups_distribution_group_name",
		mcp.WithDescription("Update one given distribution group name in an org"),
	...
	return models.Tool{
		Definition: tool,
		Handler:    Distributiongroups_patchfororgHandler(cfg),
	}
}
