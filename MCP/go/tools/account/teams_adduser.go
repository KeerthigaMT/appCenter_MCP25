...
func Teams_adduserHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	return func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		url := fmt.Sprintf("%s/v0.1/orgs/%s/teams/%s/users", cfg.BaseURL, org_name, team_name)
...
func CreateTeams_adduserTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("post_v0_1_orgs_org_name_teams_team_name_users",
		mcp.WithDescription("Adds a new user to a team that is owned by an organization"),
	...
	return models.Tool{
		Definition: tool,
		Handler:    Teams_adduserHandler(cfg),
	}
}
