...
func Distributiongroups_addusersfororgHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	return func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		url := fmt.Sprintf("%s/v0.1/orgs/%s/distribution_groups/%s/members", cfg.BaseURL, org_name, distribution_group_name)
...
func CreateDistributiongroups_addusersfororgTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("post_v0_1_orgs_org_name_distribution_groups_distribution_group_name_members",
		mcp.WithDescription("Accepts an array of user email addresses to get added to the specified group"),
	...
	return models.Tool{
		Definition: tool,
		Handler:    Distributiongroups_addusersfororgHandler(cfg),
	}
}
