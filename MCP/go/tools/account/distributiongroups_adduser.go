...
func Distributiongroups_adduserHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	return func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		url := fmt.Sprintf("%s/v0.1/apps/%s/%s/distribution_groups/%s/members", cfg.BaseURL, owner_name, app_name, distribution_group_name)
...
func CreateDistributiongroups_adduserTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("post_v0_1_apps_owner_name_app_name_distribution_groups_distribution_group_name_members",
		mcp.WithDescription("Adds the members to the specified distribution group"),
	...
	return models.Tool{
		Definition: tool,
		Handler:    Distributiongroups_adduserHandler(cfg),
	}
}
