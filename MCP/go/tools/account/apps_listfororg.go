...
func Apps_listfororgHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
...
func CreateApps_listfororgTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("get_v0_1_orgs_org_name_apps",
		mcp.WithDescription("Returns a list of apps for the organization"),
		mcp.WithString("org_name", mcp.Required(), mcp.Description("The organization's name")),
	)

	return models.Tool{
		Definition: tool,
		Handler:    Apps_listfororgHandler(cfg),
	}
}
