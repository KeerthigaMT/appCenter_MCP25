...
func OrginvitationsHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	return func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		url := fmt.Sprintf("%s/v0.1/orgs/%s/invitations/%s/revoke", cfg.BaseURL, org_name, email)
...
func CreateOrginvitationsTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("post_v0_1_orgs_org_name_invitations_email_revoke",
	...
	return models.Tool{
		Definition: tool,
		Handler:    OrginvitationsHandler(cfg),
	}
}
