...
func Apps_getteamsHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	return func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		url := fmt.Sprintf("%s/v0.1/apps/%s/%s/teams", cfg.BaseURL, app_name, owner_name)
...
func CreateApps_getteamsTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("get_v0_1_apps_owner_name_app_name_teams",
		mcp.WithDescription("Returns the details of all teams that have access to the app."),
	...
	return models.Tool{
		Definition: tool,
		Handler:    Apps_getteamsHandler(cfg),
	}
}
