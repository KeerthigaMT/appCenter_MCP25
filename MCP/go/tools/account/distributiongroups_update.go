...
func Distributiongroups_updateHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	return func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		url := fmt.Sprintf("%s/v0.1/apps/%s/%s/distribution_groups/%s", cfg.BaseURL, owner_name, app_name, distribution_group_name)
...
func CreateDistributiongroups_updateTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("patch_v0_1_apps_owner_name_app_name_distribution_groups_distribution_group_name",
		mcp.WithDescription("Updates the attributes of distribution group"),
		...
		mcp.WithBoolean("is_public", mcp.Description("Input parameter: Whether the distribution group is public")),
		mcp.WithString("name", mcp.Description("Input parameter: The name of the distribution group")),
	)

	return models.Tool{
		Definition: tool,
		Handler:    Distributiongroups_updateHandler(cfg),
	}
}
