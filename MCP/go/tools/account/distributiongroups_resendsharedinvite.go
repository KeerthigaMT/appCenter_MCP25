...
func Distributiongroups_resendsharedinviteHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	return func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		url := fmt.Sprintf("%s/v0.1/orgs/%s/distribution_groups/%s/resend_invite", cfg.BaseURL, org_name, distribution_group_name)
...
func CreateDistributiongroups_resendsharedinviteTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("post_v0_1_orgs_org_name_distribution_groups_distribution_group_name_resend_invite",
		mcp.WithDescription("Resend shared distribution group invite notification to previously invited testers"),
	...
	return models.Tool{
		Definition: tool,
		Handler:    Distributiongroups_resendsharedinviteHandler(cfg),
	}
}
