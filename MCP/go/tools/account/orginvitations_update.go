...
func Orginvitations_updateHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	return func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		url := fmt.Sprintf("%s/v0.1/orgs/%s/invitations/%s", cfg.BaseURL, org_name, email)
...
func CreateOrginvitations_updateTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("patch_v0_1_orgs_org_name_invitations_email",
		mcp.WithDescription("Allows the role of an invited user to be changed"),
		...
		mcp.WithString("role", mcp.Description("Input parameter: The user's role in the organizatiion")),
	)

	return models.Tool{
		Definition: tool,
		Handler:    Orginvitations_updateHandler(cfg),
	}
}
