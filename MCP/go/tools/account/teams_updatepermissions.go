...
func Teams_updatepermissionsHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	return func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		url := fmt.Sprintf("%s/v0.1/orgs/%s/teams/%s/apps/%s", cfg.BaseURL, org_name, team_name, app_name)
...
func CreateTeams_updatepermissionsTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("patch_v0_1_orgs_org_name_teams_team_name_apps_app_name",
		mcp.WithDescription("Updates the permissions the team has to the app"),
		...
		mcp.WithArray("permissions", mcp.Required(), mcp.Description("Input parameter: The permissions all members of the team have on the app")),
	)

	return models.Tool{
		Definition: tool,
		Handler:    Teams_updatepermissionsHandler(cfg),
	}
}
