...
func Apps_listtestersHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	return func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		url := fmt.Sprintf("%s/v0.1/apps/%s/%s/testers", cfg.BaseURL, owner_name, app_name)
...
func CreateApps_listtestersTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("get_v0_1_apps_owner_name_app_name_testers",
		mcp.WithDescription("Returns the testers associated with the app specified with the given app name which belongs to the given owner."),
	...
	return models.Tool{
		Definition: tool,
		Handler:    Apps_listtestersHandler(cfg),
	}
}
