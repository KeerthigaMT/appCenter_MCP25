...
func Teams_listallHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
...
func CreateTeams_listallTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("get_v0_1_orgs_org_name_teams",
		mcp.WithDescription("Returns the list of all teams in this org"),
		mcp.WithString("org_name", mcp.Required(), mcp.Description("The organization's name")),
	)

	return models.Tool{
		Definition: tool,
		Handler:    Teams_listallHandler(cfg),
	}
}
