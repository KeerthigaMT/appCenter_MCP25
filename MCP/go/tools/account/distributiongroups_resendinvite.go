...
func Distributiongroups_resendinviteHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	return func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		url := fmt.Sprintf("%s/v0.1/apps/%s/%s/distribution_groups/%s/resend_invite", cfg.BaseURL, owner_name, app_name, distribution_group_name)
...
func CreateDistributiongroups_resendinviteTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("post_v0_1_apps_owner_name_app_name_distribution_groups_distribution_group_name_resend_invite",
		mcp.WithDescription("Resend distribution group app invite notification to previously invited testers"),
		...
		mcp.WithArray("user_emails", mcp.Description("Input parameter: The list of emails of the users")),
	)

	return models.Tool{
		Definition: tool,
		Handler:    Distributiongroups_resendinviteHandler(cfg),
	}
}
